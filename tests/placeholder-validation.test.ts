...
describe('Placeholder Type Validation', () => {
  ...
  it('should detect placeholder type conflicts between locales', () => {
    ...
    const fallbackData = {
      ...
      MIXED_TYPES: 'User {userId} has {balance:number} points and status {isActive:boolean}',
      TYPE_CONFLICT: 'Value: {value:string}, Count: {count:number}'
    };
    
    const enData = {
      WELCOME_USER: 'Welcome {firstName} {lastName}! Age: {age:number}',
      MIXED_TYPES: 'User {userId:number} has {balance:number} points and status {isActive:boolean}',
      TYPE_CONFLICT: 'Value: {value:number}, Count: {count:boolean}'
    };
    
    const jaData = {
      WELCOME_USER: 'こんにちは {firstName} {lastName}さん、{age:number}歳ですね',
      MIXED_TYPES: 'ユーザー{userId:boolean}の残高は{balance:string}ポイント、ステータスは{isActive:number}です',
    ...
    expect(generatedCode).toContain('Warning: Placeholder types do not match across locales');
    expect(generatedCode).toContain('userId: fallback.json: string, en.json: number, ja.json: boolean');
    expect(generatedCode).toContain('value: fallback.json: string, en.json: number, ja.json: boolean');
  ...
  it('should handle implicit types correctly', () => {
    ...
    const enData = {
      IMPLICIT_TEST: 'Order {orderId:number} for {amount:number} dollars is {status}', // explicit types
    };
    ...
    expect(generatedCode).toContain('Warning: Placeholder types do not match across locales');
    expect(generatedCode).toContain('orderId: fallback.json: string, en.json: number');
  ...
  it('should handle invalid JSON files and generate warnings', () => {
    ...
    expect(generatedCode).toContain('Warning: Failed to load the following locale files');
    expect(generatedCode).toContain('invalid.json');
    expect(generatedCode).toContain('broken.json');
    expect(generatedCode).toContain('These files are not included in the generated code');
  ...
  it('should not generate warnings when all types are consistent', () => {
    ...
    const enData = {
      CONSISTENT_MESSAGE: 'User {userId:number} balance: {balance:number} points',
    };
  ...
  it('should detect conflicts between implicit string and explicit types', () => {
    ...
    expect(generatedCode).toContain('Warning: Placeholder types do not match across locales');
    expect(generatedCode).toContain('date: fallback.json: string, en.json: date');
    expect(generatedCode).toContain('userId: fallback.json: string, en.json: boolean');
  ...
  it('should reproduce and fix the demo FORMATTED_DATE issue', () => {
    ...
    const enData = {
      FORMATTED_DATE: 'Today is {date:date} and the temperature is {temperature:number}°C',
    };
    ...
    expect(generatedCode).toContain('Warning: Placeholder types do not match across locales');
    expect(generatedCode).toContain('date: ja.json: string, en.json: date, fallback.json: date');
